...
export const ActionColor = () => {
  ...
  const rawColorValue = selectedLayer?.cssVars?.["--color"];
  ...
  const handleRemoveBackgroundColor = () => {
    if (selectedLayer) {
      designerAction({
        type: "UPDATE_LAYER_CSS",
        payload: { id: selectedLayer.id, css: { "--color": "" } },
      });
    }
  };

  return (
    <DesignAction label="Color">
      <Popover.Root>
        <Popover.Trigger
      ...
      <Button
        ...
        disabled={!selectedLayer?.cssVars?.["--color"]}
